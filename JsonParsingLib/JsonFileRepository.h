...
{
public:
    explicit JsonFileRepository(std::string filePath, std::string idField = "id");

    JsonValue Create(JsonValue record) override;
    const JsonValue::ArrayType& ReadAll() const override;
    const JsonValue* ReadById(const std::string& id) const override;
    bool Update(const std::string& id, const JsonValue::ObjectType& fields) override;
